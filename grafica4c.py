...
def hashtag_visualization():
    ...
    ds=spark.sql("select idParticipant, sum(total) as suma from id1 \
    where timestamp between cast('{}' as timestamp) - INTERVAL 1 HOUR and cast('{}' as timestamp) \
    group by idParticipant order by suma desc limit 10".format(interval_time, interval_time))
    s=ds.toPandas()
    showgrafic(s)
   # ds.show()

#ds=spark.sql("select idParticipant, sum(total) as suma from id1  group by idParticipant order by suma desc limit 5")

def showgrafic(s):
    #explode = (0, 0.1, 0, 0)
    labels=s['idParticipant']
    values=s['suma']
    total = sum(values)
